...
def createTVData(file, edctv = None, idctv = None):
    ...
    print(f"stoi before = {edcacvr.string_to_id}")
    print(f"itos before = {edcacvr.vocab_map}")
    print(f"stoi len before = {len(edcacvr.string_to_id)}")
    print(f"itos len before = {len(edcacvr.vocab_map)}")
    edc, idc, stmd, stmdl = get_nested_data(idc.data["solution"], edc, idc)
    print(f"itos after = {edcacvr.vocab_map}")
    print(f"stoi after = {edcacvr.string_to_id}")
    print(f"itos len after = {len(edcacvr.vocab_map)}")
    print(f"stoi len after = {edcacvr.string_to_id}")
    print(f"stmd len = {len(stmd)}\n")

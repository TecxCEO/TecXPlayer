...
if name == "main":
    ...
    for idx, (live_tokens, _) in enumerate(token_generator):
        if idx < 20:
            print(f"  Slot [Output Position {idx+1:02d}/20] -> Streaming Parallel Tokens: {live_tokens}")
        else:
            print(f"  Slot [Closing Boundary EOS ] -> Streaming Parallel Tokens: {live_tokens}")

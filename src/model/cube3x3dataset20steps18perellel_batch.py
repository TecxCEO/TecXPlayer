# Cube3x3 traning dataset by Serial Sequential Move and parallel batches
import json
    ...
    if isinstance(data, dict):
      ...
      if len(data) < 20 and len(moves_history) <= self.max_steps:
        ...
        for key, value in data.items():
          if len(data) ==19 and key!="state" and moves_history:
...
if __name__=="__main__":
  ...
  for char in s.solve(state_given_to_solve):
    ...
    for i in range(l):
      print(f" i = {i}")
      print(f"Length of data_batch as char keys = {char.keys()} ")

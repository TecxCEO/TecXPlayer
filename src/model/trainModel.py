...
def get_nested_data(data, edc, idc):
    ...
    for key, value in data.items():
        #####
        st_mv_data = data['state'] if data.get('state') and len(data) == 19 else []
        ...
        if value.get('state') and len(value) in (16, 19):
            ...
            if st_mv_data:
                ...
                stoi, itos = edc.createTokens(st_mv_data[2])
        #####
        elif key != 'state' and len(value) == (19, 16):
           # edc, idc, smd, smdl = get_nested_data(value, edc, idc)
        ...
        for smd in st_mv_data:
            ####stoi, itos = edc.createTokens(smd[0])
            #####stoi, itos = edc.createTokens(smd[2])
